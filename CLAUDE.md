...
- **propolis is a tagged dependency (v0.0.6)**: Fetched via normal Go module resolution. `build-dev` embeds propolis-runner, libkrun, and libkrunfw into the binary (downloaded via `task fetch-runtime`/`task fetch-firmware`). Use `build-dev-system` for the old libkrun-devel path.
